...
class WC_Swatch_Picker {
	...
	public function render_picker( $name, $options, $real_name = '' ) {
		...
		<?php if ( $layout == 'label_above' ) {$label_above_styless="showlable";}else{$label_above_styless="showlable_hidden";}?>
		...
		<div 
			...
			class="select attribute_<?php echo $st_name; ?>_picker <?php echo $label_above_styless; ?>">
			...
			<?php if ( is_array( $options ) ) : ?>
				...
				if ( taxonomy_exists( $taxonomy_lookup_name ) ) :
					...
					foreach ( $terms as $term ) :
						...
						if ( $this->swatch_type_options[$lookup_name]['type'] == 'term_options' ||$this->swatch_type_options[$lookup_name]['type'] == 'text') {
						...
						} elseif ( $this->swatch_type_options[$lookup_name]['type'] == 'product_custom' ||$this->swatch_type_options[$lookup_name]['type'] == 'text') {
