...
class WC_Swatch_Term {
    ...
    public function get_output($placeholder = true, $placeholder_src = 'default') {
        ...
        $picker = $color_sstyle=$term_labelss= '';
        ...
        if($this->type == 'photo' || $this->type == 'image'||$this->type == 'color'){$term_labelss='<p>'.$this->term_label.'</p>';}
        if ($this->type == 'photo' || $this->type == 'image') {
            $picker .= '<a href="' . $href . '"  title="' . $this->term_label . '" class="' . $anchor_class . '">';
            $picker .= '<img src="' . apply_filters('woocommerce_swatches_get_swatch_image', $this->thumbnail_src, $this->term_slug, $this->taxonomy_slug, $this) . '" alt="' . $image_alt . '" class="wp-post-image swatch-photo' . $this->meta_key() . ' ' . $image_class . '"/>';
            $picker .= '</a>'.$term_labelss.'<i class="swatch-icon"></i>';
        } elseif ($this->type == 'color'||$this->type == 'text') {
			if($this->type == 'color'){ $color_sstyle='style="text-indent:-9999px;width:32px;height:32px;background-color:' . apply_filters('woocommerce_swatches_get_swatch_color', $this->color, $this->term_slug, $this->taxonomy_slug, $this) . ';"';$text_class=$anchor_class;}else{$text_class="swatch-anchor swatch-text";}
			
            $picker .= '<a href="' . $href . '"'.$color_sstyle.'  title="' . $this->term_label . '" class="' . $text_class. '">' . $this->term_label . '</a>'.$term_labelss.'<i class="swatch-icon"></i>';
        } elseif ($placeholder) {
            if ($placeholder_src == 'text') {
