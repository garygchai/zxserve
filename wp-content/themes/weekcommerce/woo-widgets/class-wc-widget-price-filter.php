 ...
 * @extends  WC_Widget
 */
class WC_Widget_Price_Filter_t extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_name        = __( '[产品筛选]价格筛选', 'woocommerce' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
	 ...
	 $ulform='id="ulform"';
		...
		if ( $_chosen_attributes = WC_Query::get_layered_nav_chosen_attributes() ) {
			foreach ( $_chosen_attributes as $attribute => $data ) {
				...
				$fields .= '<input id="' . esc_attr( $taxonomy_filter ) . '" type="hidden" name="' . esc_attr( $taxonomy_filter ) . '" value="' . esc_attr( implode( ',', $data['terms'] ) ) . '" />';
...
$show_btn='<div class="show_btn"><span> '.get_themepark_option('product_d7','展开全部').' </span></div>';
		echo $show_btn.'<div '.$ulform.' class="widget woocommerce widget_price_filter"><div class="widget_title_t">'.$instance['title'].'：</div><ul id="widget_price">';
	...
	if( !$min_price){ $min_price=esc_attr( apply_filters( 'woocommerce_price_filter_widget_min_amount', $min ) );}
	if( !$max_price){ $max_price=esc_attr( apply_filters( 'woocommerce_price_filter_widget_min_amount', $max ) );}
	...
	echo '<form method="get" action="' . esc_url( $form_action ) . '">
		<div id="price_slider_form" class="price_slider_wrapper">
					...
					<button type="submit" class="button">' .get_themepark_option('product_l3','点击筛选') . '</button>
